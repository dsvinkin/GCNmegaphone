...
# Most_Likely_Ind
gbm_trigger_classification_index_good = {
    '4': '4  GRB', 
    '5': '5  GENERIC_SGR', 
...
gbm_trigger_classification_index_all = {
    '0': '0 ERROR', 
...
class notice:
    ...
    def print_param(self, key, f):
        ...
        if (key == 'Packet_Type'):
            ...
            print("{:22s} {:s}".format(name, gbm_trigger_classification_index_good[val]), file=f)
    
        elif (key == 'Sec_Most_Likely_Index'):
            print("{:22s} {:s}".format(name, gbm_trigger_classification_index_all[val]), file=f)
...
class notice_json(object):

     def __init__(self, payload):

        self.payload = payload
        self.dict_par = json.loads(payload)
...
def check_gbm_classification(data: notice) -> (str, bool):

    MOST_LIKELY_IND = data.get_value('Most_Likely_Index')

    if MOST_LIKELY_IND is not None:
        if MOST_LIKELY_IND not in gbm_trigger_classification_index_good:
            return gbm_trigger_classification_index_all.get(MOST_LIKELY_IND, 'N/A'), False
        else:
            return gbm_trigger_classification_index_all.get(MOST_LIKELY_IND, 'N/A'), True
    else:
        return False
