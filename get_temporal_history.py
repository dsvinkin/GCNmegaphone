...
def delimitation(folder, file):

	if folder != '':

		folder = folder + '/'

	file_detectors = get_files(folder = folder)

	detectors = get_detectors(folder, file_detectors)

	resolution = [[-1, 1, 2], [-5, 50, 16], [-10, 60, 64], [-10, 100, 256]]

	GRB_data = get_files(folder = folder, pattern = '_FER.txt', switch = 1)
...
def get_files(folder, pattern = 'glg_trigdat_all', switch = 0):
...
if __name__ == "__main__":
	...
	first_tte_file = get_files(folder = folder_path, pattern = 'glg_tte_n')
